...
**Three seams, each with two adapters** — `Embedder` and `VectorStore` in `app/ports.py` (top level, because retrieval crosses them too, not just ingestion), `JobStore` in `app/ingestion/jobs.py`. Production adapters live in `app/services`: `OpenRouterEmbedder`, `PineconeVectorStore`, plus `InMemoryJobStore`; the test fakes are in `tests/conftest.py`. This is why the suite needs no network. Don't add a port without a second adapter to justify it.

**The vector record shape belongs to `app/services/vector_store.py` and nowhere else** — id derivation (`sha1(source)[:16]` prefixing `{doc_id}:{i}`), namespace-by-folder, and the metadata keys all live there, along with `search()` which decodes them back into `Chunk`s. Nothing outside that module should construct or read a Pinecone record; assert on the record shape only in `tests/test_vector_store.py`. Connection lifecycle is separate, in `app/services/pinecone.py`.

Ingestion **skips** Documents whose Chunks are already stored, decided **before conversion** (`Ingestor._ingest` resolves, checks `stored_chunks` per source, then converts only the rest) so a skip costs one listing rather than a trip through Docling. `IngestionService.convert(sources)` takes an explicit list for this reason; `process()` is resolve + convert, for Preview.

**`document.source` must be the resolved input path, never Docling's `result.input.file`** — Docling reports only the basename. `convert()` zips results back to the input list (convert_all preserves order; a name check guards it). Using the basename silently broke namespace-by-folder (`Path('x.md').parent.name == ''`), collided ids for same-named files in different folders, and stored filenames as `source` metadata. `IngestionRequest.replace` forces a re-embed and calls `forget()` first, so a Document that shrank leaves no orphaned Chunks behind.
...
- **OpenRouter chat** (`app/services/llm.py`) — an `AsyncOpenAI` client pointed at OpenRouter's base URL, built lazily via `@lru_cache` on first call. Model ids are OpenRouter-style (`openai/gpt-4o-mini`).
- **OpenRouter embeddings** (`app/services/embedder.py`) — a *separate* `@lru_cache`d client with its own timeout, so embeddings can point at another provider without touching chat; `EMBEDDING_API_KEY`/`EMBEDDING_BASE_URL` fall back to the OpenRouter values when blank. Raises its own `EmbedderConfigError`. `OpenRouterEmbedder` batches internally (`EMBEDDING_BATCH_SIZE`, default 96) and sorts responses by `index` — do not assume the provider returns them in order. Don't call it directly from new code: take an `Embedder` through the seam.
- **Pinecone** (`app/services/pinecone.py`) — one `AsyncPinecone` client + index held for the whole process, opened in the `lifespan` in `app/main.py` and stashed on `app.state.vector_index`. Both objects own HTTP connection pools and must be closed, so never open one per request. Routes reach ingestion through `get_ingestor`; `get_vector_index` is the dependency for future routes that need the raw index. Nothing here computes embeddings — Pinecone is storage/retrieval only.
